...
def transmit_region_frame(region, channel_freq, node_id):
...
if __name__ == "__main__":
    print(f"[*] Initializing WA, SA, and ACT Sovereign RF Line Protocol...")
    regions = ["WA-Edge", "SA-Edge", "ACT-Edge"]
    channels = [915.0, 868.0, 433.5, 2412.0]
    
    node_counter = 1
    for region in regions:
        for ch in channels:
            transmit_region_frame(region, ch, node_counter)
            node_counter += 1
            time.sleep(0.05)
            
        print(f"[+] {region} Transceiver Matrix Locked.")
        
    print("[+] All National Edge Nodes Synchronized Across Frequency Lines.")
