...
def convert_red_hat_to_nft():
    print("[*] Minting Red Hat namespace into sovereign ERC-721 structure...")
    
    git_head = get_git_commit_hash()
    asset_name = "Red Hat"
    symbol = "REDHAT"
    token_id = 999  # Sovereign reserved index
    ...
    erc721_metadata = {
        "name": "Red Hat Sovereign Asset",
        "symbol": symbol,
        "description": "Sovereign cryptographic representation of Red Hat namespace under RobDoe Pty Ltd authority.",
        "token_id": token_id,
        "namespace": "redhat.atom",
        ...
        "attributes": [
            {"trait_type": "Asset Class", "value": "Enterprise Namespace"},
            {"trait_type": "Authority", "value": "ATOM-TRUTH"},
            {"trait_type": "Jurisdiction", "value": "Australia"}
        ]
    }

    os.makedirs("redhat_nft", exist_ok=True)
    with open("redhat_nft/redhat_token.json", "w") as f:
        json.dump(erc721_metadata, f, indent=2)

    print("==================================================")
    print("   RED HAT ERC-721 ASSET LOCKED")
    print("==================================================")
    print(f"[+] Asset Name         : {asset_name}")
    print(f"[+] Token ID           : {token_id}")
    print(f"[+] Git Hash Proof     : {git_head[:16]}...")
    print(f"[+] Individual Root    : 0x{cryptographic_root_hash}")
    print(f"[+] Saved To           : ~/robdoerootauthority/redhat_nft/redhat_token.json")
    print("==================================================")

if __name__ == "__main__":
    convert_red_hat_to_nft()
